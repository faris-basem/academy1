...
@section('content')
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="table yajra-datatable" id="questions-table">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>السؤال</th>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel33"> إضافة سؤال</h4>
                    ...
                    <form id="add_question_form" class="invoice-repeater">
                        ...
                        <div class="modal-footer">
                            <button type="button" style="display: none" id="add_question2"
    ...
    <div class="form-modal-ex">
        <div class="modal fade text-left" id="edit_question" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33"
            aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel33"> تعديل سؤال</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="edit_question_form">
                        ...
                        <div class="modal-footer">
                            <button type="button" style="display: none" id="edit_question2"
                                class="btn btn-primary btn-block">جاري التعديل ...</button>
                            <button type="button" id="edit_question1" class="btn btn-primary btn-block" onclick="do_edit()">تعديل</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    {{-- delete question --}}
    <div class="modal fade modal-danger text-left" id="delete_question" tabindex="-1" role="dialog" aria-labelledby="myModalLabel120" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="delete_question_form">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" id="delete_question2" style="display: none" data-dismiss="modal">...يتم الحذف</button>
                            <button type="button" class="btn btn-danger" onclick="do_delete()" id="delete_question_button" data-dismiss="modal">تأكيد</button>
                        </div>
                    </form>
            </div>
        </div>
...
<script>
    $(document).on('click', '#add_question', function(e) {
        ...
        $.ajax({
            ...
            url: "{{ route('store_question',['quiz_id'=>$quiz_id]) }}",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
...
<script type="text/javascript">
    ...
    $(function () {
        var table = $('#questions-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get_questions_data',$quiz_id) }}",
            columns: [
    ...
    <script>
        $('#edit_question').on('show.bs.modal', function(event) {
            ...
            modal.find('.modal-body #select2').val(quiz_id);
    ...
    <script>
        function do_edit(){
            $("#edit_question1").css("display", "none");
            $("#edit_question2").css("display", "block");
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('.yajra-datatable').DataTable().ajax.reload(null, false);
                    $("#edit_question2").css("display", "none");
                    $("#edit_question1").css("display", "block");
                    $('.close').click();
